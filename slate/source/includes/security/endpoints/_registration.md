### 19.10. Dynamic Client Registration endpoints
...
For additional statements on the operation of these endpoint during client registration see the [Client Registration](#6-client-registration) section.
...
1. During registration management requests, Data Holders **MUST** validate that the scope of access tokens provided includes `cdr:registration`.
1. Registration requests and responses must conform to the specification in the [DCR APIs](#dcr-apis) section.
1. Any fields the Data Holder does not support **MUST** be ignored without error.
1. Registrations **MUST** only be updated via a PUT operation on the registration endpoint.
1. POST and PUT operations **MUST** accept the SSA payload.
1. Update (PUT) operations are to be used for one of two scenarios:
  ...
  1. A new version of the SSA has been released and the client updates this information to the data holder brands.
