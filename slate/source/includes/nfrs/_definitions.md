...
```diff
Updated the definition of 'Customer Present'
- the x-fapi-customer-ip-address header is populated with a valid IP address of the end customer's device
+ the x-fapi-end-user-present header value is true
```

In the following definition of NFRs specific terms have the following meanings:

- **Data Recipient Software Product**: For the purposes of these NFRs a Data Recipient Software Product is defined, inline with the definition given in the [CDR Federation](#2-cdr-federation), as a configured application presented in the register meta data. This acknowledges that a single accredited entity may be able to register multiple independent services (or apps) that can obtain authorisations from consumers independently of each other.
